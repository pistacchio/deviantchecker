...
deviantCHECKER is first of all an exercise in futility: in fact it reproduces a function already available on **[Deviantart](http://www.deviantart.com/)**! Deviantart is an online community for sharing works of art where people showcase their drawings, photography and the like. deviantCHECKER is a personal (it doesn't even have authentication) application that lets you add artists you'd like to follow and automatically checks whenever they post new works.
...
* Lein-Ring is Leiningen plugin that interfaces with Ring and lets up start a _development_ server that makes our debugging and testing much easier. `:dev-dependencies` are only resolved at build time, not at runtime. Lein-Ring is not part part of our project, we are just using it during the development.
* `:ring` is a special Lein-Ring (you guessed it!) key that lets us specify the handler for the requests (more on this later)
...
The whole web part of our application is contained within `/src/org/github/pistacchio/deviantchecker/core.clj`. The _namespace_ part part looks like this:
...
`(defroutes)` lets us define how calles are despatched. `main-routes` is how we named our route dispatcher. `(GET "/" [] (get-home))` instructs the server so that everytime it receives a _GET_ call to `http://127.0.0.1:3000/` it must call `(get-home)`. The first param can of course be as well _POST_, _DELETE_, _PUT_, _HEAD_ as they are valid http methods, or _ANY_, meaning that you can call address the call with whatever method.
...
We're almost done! `(route/not-found "Page not found")` will return "Page not found" whenever someone tries to visit a page we've not defined as a valid route or an internal server error accours. Time for a fancier 404 page?

## Data

### Serialization

We are storing all the information about the Deviantart galleries in a plain text file: `/resources/data/data.dat`. In a real world application, of course, a database backend would be compulsory, but for our example it is enough.
...
I won't go into details about our data layer because you can easily read about it through the code. Breafly, data about galleries is stored (in memory and on the file) as a sequence of maps. Each map represents a gallery. For each gallery we story:

* the base url of the gallery into the key `:href` (http://USERNAME.deviantart.com/gallery)
* the url of the last page of the gallery in case of multi-pages gallieries into `:last-page` (for example http://USERNAME.deviantart.com/gallery?offset=12)
* the number of images on the last page into `:num-images`.

Doing so, it is easy to check whenever a user of Deviantart has added new works: we just need to retrieve again the data about a certain gallery and see if `:last-page` or `:num-pages` that we've stored are different!
...
You can produce the whole page in the form of a string, but keeping html separated from the code is always a good idea and that's why we use Enlive. There are other template engines available for Clojure, for example [Fleet](https://github.com/Flamefork/fleet) or [Hiccup](https://github.com/weavejester/hiccup) (that comes shipped with Compojure) plus, of course, a plethora of Java templates engines.
...
As you can see, it is possible to modify and analyze this kind of structure, but fortunatly Enlive provides us with a handful of functions that make it easier to work with this kind of heavily nested structure.
...
While developing, this will give you some invaluable helps, notable:

* It monitors the application files and whenever you make some changes, it realoads the application so that you don't need to restart the server to see every little change done to the code
* It shows a stacktrace of any error in the browser
...
Leiningen can also pack it all into a _.war_ file with `lein ring war` and `lein ring uberwar` and deploy it under [Tomcat](http://tomcat.apache.org/) or any other Java server supporting wars. I haven't tested this already, but when I'll do I'll update the tutorial.
